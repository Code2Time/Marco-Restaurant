...
const {HandleDecreasOrder , HandleIncreasOrder , HandleRemoveOrder} = UseOrdersContext();
...
const subtotal:any = SelectedOrders?.price
  ...
  return (
    ...
    <button onClick={()=> HandleRemoveOrder(id)} className="hidden sm:block sm:size-5 cursor-pointer hover:text-red1 " >  <FaTrash/></button>
      ...
      <div className="col-span-12 text-center ">
        ...
        <div className="flex flex-col justify-center items-center mt-5 gap-7">
          <div className="flex justify-center items-center mt-5 gap-7">
            <button onClick={()=>HandleDecreasOrder(id)} className="border p-3 text-my-white bg-red1 rounded cursor-pointer hover:shadow-lg shadow-red1">
              <FaMinus />
            </button>
            <h1 className="poppins-7 text-4xl">{qty}</h1>
            <button onClick={()=>HandleIncreasOrder(id)}  className="border p-3 text-my-white bg-red1 rounded cursor-pointer hover:shadow-lg shadow-red1">
              <HiOutlinePlus />
            </button>
          </div>
          <button onClick={()=>HandleRemoveOrder(id)}  className="border w-full py-3 bg-red1 text-my-white rounded-md overflow-hidden cursor-pointer sm:hidden">
