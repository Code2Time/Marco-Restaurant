      ...
      <div className="footer-container w-full h-auto flex flex-col justify-center items-center mt-40 text-my-white">
        <div className="w-full h-auto item1 grid grid-cols-12 bg-red1">
          ...
          <div className="news-letter col-span-12 sm:col-span-6 lg:col-span-4 p-10 sm:p-12 text-xs sm:text-base">
            ...
            <p className="text-slate-200 capitalize my-2">
              You can trust us. we only send promo offers, not a single spam.
            </p>
            <div className="w-full relative p-4">
              <input
                type="text"
                className="w-full h-7 sm:h-10 px-4 py-2 rounded-xl placeholder:text-slate-800 order-1 focus:outline-none text-center text-black focus:outline-slate-400"
                placeholder="Your email address..."
              />
              <button
                type="submit"
                className="absolute right-[17px] top-4 sm:top-5 sm:right-5  order-2 rounded-full bg-red1 p-2 text-white"
              >
                <FaArrowRight />
              </button>
            </div>
        ...
        <div className="item2 w-full h-auto bg-gray-4 flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-10 p-3">
          <div className="copy-right ">
            <p className="flex items-center gap-1 poppins-5 text-xs">
              Copyright ©2024 | This template is made with{" "}
              <span>
                <CiHeart color="red" className="cursor-pointer" />
              </span>{" "}
              by <span className="text-red1 cursor-pointer">Amir</span>
            </p>
          </div>
